...
- To run with NX CLI install nx: `npm install -g nx`
- Install local dev dependencies: `npm install` while current directory is this repo

### Development server

- Run `npm start CalendarApp` or `nx run CalendarApp:serve` to start a development server for CalendarApp SPA which have both Work Hours & SignUp Sheet functionalities using TimeRangePickerLib Library.
- Run `npm start demos` or `nx run demos:serve` to start a development server for demos App for Angular Calendar Library.

### Testing

- Run `npm test TimeRangePickerLib` or `nx run TimeRangePickerLib:test` to run tests for TimeRangePicker Library.
- Run `npm test CalendarApp` or `nx run CalendarApp:test` to run tests for CalendarApp SPA.
- Run `npm test angular-calendar` or `nx run angular-calendar:test` to run tests for Angular Calendar Library.
- Run `npm run test:single` to run & generate tests with coverage report for TimeRangePicker Library.

### Complete Workspace Graph view

- Run `nx graph` will open browser window, we can view complete grahical view of all the Apps, Libraries in this workspace with interlinking between those in high level.
